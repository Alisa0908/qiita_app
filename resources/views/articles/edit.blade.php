...
<body>
    ...
    <form action="{{ route('articles.update', $article->id) }}" method="POST">
        ...
        <div>
            <input type="text" name="title" id="title" placeholder="タイトル" value="{{ $article->title }}" required>
        </div>
        <div>
        ...
        <div>
            <textarea name="body" id="body" cols="30" rows="10"
                placeholder="エンジニアに関わる知識をMarkdown記法で書いて共有しよう" required>{{ $article->body }}</textarea>
        </div>
